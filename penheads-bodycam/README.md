...
# Optional: Windows Bodycam Companion (Electron) — NUI POSTs start/stop to localhost; see bodycam-companion README.
# setr bodycam_companion "1"
# setr bodycam_companion_url "http://127.0.0.1:4555"
# Optional: one MP4 — NUI clip is video-only, companion captures loopback+mic, companion FFmpeg-muxes on stop (best-effort sync).
# setr bodycam_companion_mux_clip "1"
```
...
- **First person:** `UseFirstPersonForClipRecording` (default **true**) makes **saved WebM frames** first-person. `ClipFirstPersonHoldWhileRecording` (default **true**) controls the **player camera** during the clip burst: **true** = one first-person hold for the whole capture (no third/first **flicker**); **false** = switch FP **every frame** (strobing while the WebM saves — usually undesirable). Set `ClipFirstPersonRequiresSnapshotToggle` to **true** to require the same personal first-person toggle from `/bcamconfig` for clip angles.
...
- **Companion mux (`bodycam_companion_mux_clip` + companion running):** When enabled, the clip WebM is **video-only** from game frames; **loopback + mic** come from the **Bodycam Companion** exe (WASAPI). On bodycam off, NUI sends the WebM to the companion, then stop; the companion muxes into **one H.264+AAC MP4** and completes the evidence upload. Mic/display audio modes in NUI for the clip are bypassed for that path. Alignment is **best-effort** (FFmpeg **`-shortest`**); use modest pre-roll and the same session boundaries for predictable results.
- **Microphone:** When `EnableClipRecordingMicrophone` is true (default), NUI calls **`getUserMedia({ audio })`** and mixes that track into the WebM. That is the player’s **physical microphone** in Windows (what Discord/browser would use). Chromium may show a **permission prompt** the first time; **`ClipMicrophoneWarmupOnActivate`** (default **false**) controls whether that happens **when bodycam turns on** vs only when the **WebM clip** starts (turn bodycam off with clip mode on). **Ignored** when companion mux is active (audio is from the exe).
...
- **Game + voice chat on clips (`ClipAudioCaptureMode`):** FiveM does **not** expose raw engine or Mumble PCM to Lua. This resource can still record **what you hear** by using the browser’s **`getDisplayMedia`** screen share with **system / monitor loopback audio** (Chromium on Windows: pick the monitor running GTA and enable **Share audio**). That captures **game output** and **positional voice** as long as they play through the normal Windows playback device (headphones/speakers). Modes:
  ...
  - **`display_plus_mic`** (default in `config.lua`) — **loopback + microphone** mixed in NUI (Web Audio): captures what plays on the chosen **monitor** (usually the same mix as **default output / headphones**) plus your mic. Use `setr bodycam_clip_audio_mode mic` if CEF cannot do screen capture.
...
- **Max audio without installing extra apps (recommended stack):**
  1. Default targets **game / default output** via **`display_plus_mic`**; run **`bodycamclipaudio`** once (monitor + **Share audio**). If **`Invalid state`** persists, use **`mic`** mode or route game audio into a virtual input (`bodycam_mic_devices` + `ClipMicrophoneDeviceId`). Stay on a **current** FiveM client build.
